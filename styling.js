...
const ccWhite =         ['white'];
const ccpuppy =        ['#ed9645', '#c3580d', '#de5a01', '#974810'];
...
function toast(
    ...
    let istutorial = (title.toUpperCase().includes('TUTORIAL') || title.toUpperCase().includes('puppy CLICKER'))
 ...
 */
//#region 
function popupHandler(useMousePos = true, amount, style = 'puppy') {
    ...
    // Get position of puppy image (used when useMousePos is false)
    var mcPosition = mainpuppy.getBoundingClientRect();
    ...
    let text = '';
    // puppy
    if(style === 'puppy') text = `${sign}${amount}`; // Normal click
    else if(style === 'falling')  text = `${sign}${amount}`; // Falling puppy
...
function setCosmetic(target, to, resetState = false) {
    ...
    switch(target) {
        ...
        // Farmable
        case 'farmable':
            if(cosmetic.image) mainpuppy.src = cosmetic.image; // Image
            if(cosmetic.farmable) nameLoop(cosmetic.farmable); // Name
            else nameLoop('puppy');
            if(cosmetic.render_type === 'pixel') mainpuppy.classList.add('render_pixelated'); // Image render type (Pixelated)
            else mainpuppy.classList.remove('render_pixelated');
            break;
        // Tools
        case 'tools':
...
function populateThemeList() {
    ...
    for(let i = 0; i < themesKeys.length; i++) {
        ...
        // Unlocked HTML
        let imgsrc = theme.image !== false ? theme.image : './assets/puppy Clicker.png';
        themeHTML += /* html */
        `
...
function populateCosmeticsList(target='all') {
    ...
    for(let i = 0; i < list['keys'].length; i++) {
        ...
        let imgsrc = cosmetic.hasOwnProperty('preview') ? cosmetic.preview : (cosmetic.hasOwnProperty('image') ? cosmetic.image : './assets/puppy Clicker.png');
...
function populateAchievements(specific=false) {
    ...
    if(achievementHTML === '') {
        if(filter === 'unlocked') {
            achievementHTML = `<center><img src="./assets/theme/pixel_puppy.png" class="footer_puppy"><br/><p class="secondary_text">No achievements yet. :(</p></center>`;
        }
        // Filter by locked
        else if(filter === 'locked') {
            achievementHTML = `<center><img src="./assets/piggy_bank.png" class="footer_puppy"><p class="secondary_text">You've unlocked every achievement- great job!</p></center>`;
        }
        // Filter by secret
        else if(filter === 'secret') {
            achievementHTML = `<center><img src="./assets/easter_egg.png" class="footer_puppy pointer" onclick="mouseConfetti([24,24], confettiColors, 300)"><p class="secondary_text">Don't tell anyone, but: you don't have any secret achievements.<br/>Secret achievements don't appear in the list until unlocked and<br/> they don't count towards your completion percentage.</p></center>`;
        }
    }
